...
**tasks**
- id, project_id (FK, nullable), title, status (next/in_progress/waiting/someday/done/dropped/pushed), do_date, completed_date, effort (15m/30m/1h/2h+), work_type (scheduled/deep/admin), type (home/away), notes, person_dependency, reminders_uuid (for tracking what was pushed), notion_url (for status writebacks), created_at, updated_at

  Note: `someday` is the **relabeled `triage`** status — the persisted DB value and the Notion select name both stay `triage`/`Triage` (no migration, writeback unbroken); only Course-native UI surfaces show "Someday". `pushed` = handed to Apple Reminders.
...
- **Swipe right** (>80px) → opens a compact **push date menu** anchored to the row (the row snaps back to neutral while it's open — it does *not* commit on release). Options: **Tomorrow**, **Next week** (next Monday), **Next month** (first Monday of next month), and **Pick a date** (native date input). Choosing one sets `do_date` + clears `day_order` and shows a 3-second undo toast; undo restores the prior date/order *and* `reschedule_count` so the DB forward-move trigger doesn't double-count. The option list is **data-driven** — V2 will slot an AI "smart day" recommendation (e.g. "Wed — light task day") in as one additional entry, additively, no refactor.
- **Swipe left** (>80px) → opens a compact **state menu** anchored to the row, mirroring the push menu's grammar (right = "when?", left = "what state?"). Options: **Drop** (`status='dropped'` — leaves the active views), **Waiting** (`status='waiting'` — stays visible; `person_dependency` is set later via the Task Sheet, not prompted here), **Someday** (`status='triage'` *and* clears `do_date`/`day_order` → lands in the No-date backlog; "Someday" is the relabeled-`triage` parked bucket). Each shows a 3-second undo toast restoring the prior status (and, for Someday, the prior date/order/`reschedule_count`). Marking Done is still **only** the checkbox + Task Sheet status picker — neither swipe is a Done path. *(The earlier "Mark done on swipe-left" idea was rejected; the inert old "set status=triage" swipe-left is replaced by this menu — that's the "better use" for the dead triage status.)* V2: a periodic Someday-resurfacing step in Monday Open (parallel to the monthly orphan-task review).
...
The **same swipe gestures also work on Project Detail's task rows** — the gesture layer is shared, not Today-view-only. Swiping a task in a project's task list opens the same push date menu (right) or state menu (left) identically. Both are **one body-level row-menu overlay keyed by `kind` (`push` | `state`) and task id**, so behavior is identical on both surfaces with no per-surface code.
...
- **Status** — chip + tap-to-open picker with all task statuses, shown by their UI labels (**Someday** / Next / In Progress / Waiting / Pushed / Done / Dropped — "Someday" is the relabeled `triage` value). Save writes to Supabase + writes back to Notion's `Task Status` (the `triage` value still writes back as Notion's `Triage` select; `Complete=true` on Done).
...
**Tasks section** — Header: "Tasks · 6 open" + toggle between **By status** (In Progress / Next / Waiting / **Someday** as its own visible group; Done/Dropped collapsed together) and **By date** (Today / This week / Later / No date). Same task row anatomy as Tasks mode dashboard. Tapping the row body (not the checkbox) opens the [Task Sheet](#task-sheet).
...
**V2 insertion point — Someday resurfacing.** Swipe-left → "Someday" parks tasks (`status='triage'`, no date). In V1 they live in the No-date backlog (visible there + the Project Detail "Someday" group + Today's Sort→Status). V2 adds a periodic **Someday review** step to Monday Open — parallel to the orphan-task review above — that surfaces aged Someday tasks for a keep/revive/drop decision, so the parked bucket can't silently become a black hole.
