...
- **Next-action surfacing on Triage cards.** Each active project card now collapses to its *next action* instead of the old top-3 task list. Selection logic (`surfaceActions` + `saDaysFromToday`/`fmtDueShort`, module scope in `triage.jsx`): Next candidate = task with `next` (status `next`) else topmost incomplete in project order; if any incomplete task is due within 3 calendar days inclusive (overdue counts), that urgent task is the primary line and the Next candidate shows as a secondary line (single line when they're the same task). A `+N` count badge + chevron expands the rest of the open tasks inline (surfaced stays pinned at top, no re-sort); expansion state persists under `triage.projExpanded`. Tapping the **project name** still opens the detail screen; the **chevron** toggles expansion. Empty projects show `No next action`. Urgent due chips render in `--risk`. SW bumped to `course-v28`. Logic unit-tested (11 cases); same feature shipped in Today's PillarBlockView. **Scope:** Triage cards only — the Project detail view is intentionally unchanged (full list is correct there).

- **Notion → Supabase project sync (`notion-sync.js`).** Root-caused "missing projects": the new UI never reads Notion for the project list — it renders the `course_projects` table, which was a one-time, partial import snapshot (42 rows vs **98 in Notion**). No live read, no re-sync. Added a manual "Pull projects" button in Settings that paginates the Notion Projects DB (non-archived: Active/Idea/Paused/Routine/Under Review/empty), resolves pillar via the Pillar rollup → pillar page title → slug, and upserts into `course_projects` keyed by Notion page id (insert new / update Notion-owned fields; preserves `sort_order` + `last_activity_at`). Done/Archived are not pulled. The correct Projects `database_id` is **`2d5706a32fa5809b8cfae74cf4d97156`** (the earlier-noted `2d57…805f…` is a stale/data-source id that 404s); the sync self-heals `localStorage.notion_projects_db_id` to the working id. Empty-status projects map to `active`; work_area is not yet synced. Known gap: projects later marked Done/Archived in Notion aren't reconciled back (filtered out of the pull).
...
- (2026-05-28) Next-action surfacing on Triage cards: `surfaceActions` logic is unit-tested (11 cases incl. urgent boundaries/overdue) and `triage.jsx` parses clean via `@babel/parser`, but the **UI was not visually clicked through**. Confirm: collapsed card shows one (or two, when urgent) task line; `+N` chevron expands/collapses the rest with the surfaced task pinned at top; project-name tap opens detail while chevron-tap does NOT navigate; urgent due chip is red; `No next action` shows for projects with no open tasks. Watch the interaction with long-press drag (both live on the card now) and that expansion state survives a Triage reload (`triage.projExpanded`). Most live tasks have no `do_date`, so the *urgent* path won't trigger until a task is due within 3 days — set one to test.
